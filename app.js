...
async function loadFromSheets() {
  ...
  // 修正從 Sheets 載入的日期格式
  EQ.forEach(e => {
    if (e.schedYM && String(e.schedYM).length > 7) {
      e.schedYM = String(e.schedYM).slice(0, 7);
    }
  });
  REC.forEach(r => {
    ...
    if (r.plan && String(r.plan).length > 7) {
      r.plan = String(r.plan).slice(0, 7);
    }
    if (r.next && String(r.next).length > 7) {
      r.next = String(r.next).slice(0, 7);
    }
  });
